...
def remove_gapped_columns(msa,site_threshold=0.95):
	...
	nogap_seqs = [Protein(nogaps_df[i].str.decode("utf-8").str.cat(), metadata = {"id":i}) for i in nogaps_df]
...
def get_reduced_alignment(fasta_filename,angio_1kp_ids,site_threshold=0.95,sample_threshold = 0.5,write_alignment=True):
	...
	msa = TabularMSA.read(fasta_filename, constructor=Protein)
	seqs_to_keep = []
	for seq in msa:
		if seq.metadata["id"].split("_")[0] in angio_1kp_ids:
...
def reduced_alignment(fasta_filename,medoids,gene):
	...
	msa = TabularMSA.read(fasta_filename,format = 'fasta',constructor=Protein)
	seqs_to_keep = []
	#print(medoids)
	for seq in msa:
...
fasta_filename = "individual_genes/{}.FAA".format(gene)
...
if os.path.isfile(degapped_alignment_fn):
	angio_msa_nogap_noshort = TabularMSA.read(degapped_alignment_fn,constructor=Protein)
	sys.stderr.write("Read in degapped alignment: {}\n".format(angio_msa_nogap_noshort.shape))
else: 
	angio_msa_nogap_noshort = get_reduced_alignment(fasta_filename,angio_1kp_ids)
...
for k,i in itertools.product(range(6,16),range(100)):
	try:
		...
		num_over_25 = len(medoid_dist[medoid_dist > 0.15])
